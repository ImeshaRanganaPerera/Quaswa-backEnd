...
partyRouter.post("/", authenticate, async (request: ExpressRequest, response: Response) => {
    ...
    try {
        ...
        const newParty = await partyService.create({ name: data.name, nic: data.nic, phoneNumber: data.phoneNumber, address: data.address, email: data.email, partyGroupId: partyGroup?.id, createdBy: userId })
...
partyRouter.put("/:id", authenticate, async (request: ExpressRequest, response: Response) => {
    ...
    try {
        ...
        const updateparty = await partyService.update({ name: data.name, nic: data.nic, phoneNumber: data.phoneNumber, address: data.address, email: data.email }, id)
